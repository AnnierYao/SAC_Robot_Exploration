...
class Robot:
    def __init__(self, index_map, train, plot):
        ...
        self.map_list = sorted(os.listdir(self.map_dir))
        ...
        self.action_space = np.genfromtxt(current_dir + '/scripts/action_points.csv', delimiter=",")
    ...
    def step(self, action_index):
        ...
        if reward <= 0.02 * Reward_Scale and not collision_index:
            reward = -0.8 * Penalty_Scale
            new_location = True
            terminal = True
        ...
        # print('map_local:', np.shape(map_local))
    ...
    def take_action(self, action_index, robot_position):
        # print(action_index)
        # move_action = self.action_space[int(action_index), :]
    ...
    def get_reward(self, old_op_map, op_map, coll_index):
        if not coll_index:
            reward = float(np.size(np.where(op_map == 255)) - np.size(np.where(old_op_map == 255))) / 14000 * Reward_Scale
            if reward > 1:
                reward = 1
        else:
            reward = -1 * Penalty_Scale
        return reward
